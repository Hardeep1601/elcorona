...
//    // THE WEIGHT IS SET TO 1 BY DEFAULT IN THE CONSTRUCTOR
//    public boolean addEge(V from, V to) {
...
//                    Edge newNode = new Edge(temp, null);
//                    Edge edgeNode = (Edge) currentNode.getEdgeLink();
//                    if (edgeNode==null)
...
//    }
    ...
    // INPUT THE WEIGHT OF THR GRAPH
    public boolean addEge(V from, V to) {
//        if(isEdge(from, to)){
//            return false;
//        }
        ArrayList<String> a=getAdjacent(from);
        for (int i = 0; i < a.size(); i++) {
            if(a.get(i).equals(to)){
                return false;
            }
        }
        E weight=(E)(Integer)1;
        if (hasVertice(from)==null || hasVertice(to)==null)
            return false;
        else {
            GraphNode currentNode = head;       
...
//        return false;
    ...
    public void checkEdge(V from, V to){
        if(!isEdge(from, to)){
